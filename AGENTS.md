...
- **Stock transfer state machine** — Draft → Approved → Picked → Dispatched → Received (terminal: stock moves on receive; `bayaan_transfer_state` has no separate "Completed" state); multi-line transfers; kiosk-receive confirmation; partial/damaged lines. Backend `/stock_transfer_action` exists; UI wiring in progress.
