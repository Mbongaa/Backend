      ...
      >
        <div className="mx-auto flex w-full max-w-(--thread-max-width) flex-1 flex-col px-4 pt-4">
          ...
          <ThreadPrimitive.ViewportFooter className="aui-thread-viewport-footer sticky bottom-0 mt-auto flex flex-col gap-4 overflow-visible rounded-t-(--composer-radius) bg-background pb-4 md:pb-6">
            <ThreadScrollToBottom label={labels.scrollToBottom ?? "Scroll to bottom"} />
            {footerBeforeComposer}
            <Composer
...
const ThreadMessage: FC<{ assistantExtras?: ComponentType; labels: ThreadLabels }> = ({
  assistantExtras,
  labels,
}) => {
      ...
      <div className="aui-thread-welcome-center flex w-full grow flex-col items-center justify-center">
        <div className="aui-thread-welcome-message flex size-full flex-col justify-center px-4">
          <h1 className="aui-thread-welcome-message-inner fade-in slide-in-from-bottom-1 animate-in fill-mode-both font-semibold text-2xl duration-200">
            {labels.welcomeTitle ?? "Hello there!"}
          </h1>
          <p className="aui-thread-welcome-message-inner fade-in slide-in-from-bottom-1 animate-in fill-mode-both text-muted-foreground text-xl delay-75 duration-200">
            {labels.welcomeSubtitle ?? "How can I help you today?"}
          </p>
        </div>
      </div>
      <ThreadSuggestions />
      ...
      <ComposerPrimitive.AttachmentDropzone asChild>
        ...
        >
          ...
          <ComposerPrimitive.Input
            ...
            className={cn(
              "aui-composer-input max-h-32 min-h-10 w-full resize-none bg-transparent px-1.75 py-1 text-sm outline-none placeholder:text-muted-foreground/80",
              inputClassName,
            )}
            rows={1}
            submitMode="enter"
    ...
    <div className="aui-composer-action-wrapper relative flex items-center justify-between">
      ...
      <AuiIf condition={(s) => !s.thread.isRunning}>
        <ComposerPrimitive.Send asChild>
          <TooltipIconButton
            tooltip={labels.sendMessage ?? "Send message"}
            side="bottom"
            type="button"
            variant="default"
            size="icon"
      ...
      <AuiIf condition={(s) => s.thread.isRunning}>
        <ComposerPrimitive.Cancel asChild>
          <Button
            ...
            aria-label={labels.stopGenerating ?? "Stop generating"}
          >
  ...
  return (
    ...
    >
      <ActionBarPrimitive.Copy asChild>
        <TooltipIconButton tooltip={labels.copy ?? "Copy"}>
      ...
      <ActionBarPrimitive.Reload asChild>
        <TooltipIconButton tooltip={labels.refresh ?? "Refresh"}>
          <RefreshCwIcon />
        </TooltipIconButton>
        ...
        >
          <ActionBarPrimitive.ExportMarkdown asChild>
            <ActionBarMorePrimitive.Item className="aui-action-bar-more-item flex cursor-pointer select-none items-center gap-2 rounded-sm px-2 py-1.5 text-sm outline-none hover:bg-accent hover:text-accent-foreground focus:bg-accent focus:text-accent-foreground">
              ...
              {labels.exportMarkdown ?? "Export as Markdown"}
