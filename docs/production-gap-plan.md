...
| --- | --- | --- |
| Stage 2 classic ERP release gate | Green | `docs/sales-meeting-gap-audit-2026-05-20.md` records green evidence for landed costs, offline opening/receipt semantics, source product images, peak-hour reporting, analytic cost centers, pricelist/discount controls, Odoo void/invoice/period locks, and MRP/semi-finished goods. |
| Stage 3 refactor gate | Green | Landed-cost controller logic has been extracted behind `BayaanLandedCostService` while keeping `/bayaan/api/landed_cost` as the public route. Two post-refactor full passes on 2026-05-21 are green: `npm run verify` passed twice, and the full Ubuntu WSL addon gate passed against DBs `bayaan_codex_20260521_031600` and `bayaan_codex_20260521_032943`; the final pass reported 85 post-tests, 111 addon tests, 0 failed, 0 errors. |
| Frontend scripted gate | Green | `npm run verify` passed on 2026-05-21 with 171 Vitest tests, wiring gate, production build, and Playwright smoke. The smoke includes active admin sections, major drill-downs, dark mode, Arabic RTL, mobile render, POS sale/payment/waste, screenshot capture, and console/page-error checks. |
...
| Dashboard component parity gate | Green by release walkthrough coverage | The active Vite runtime still contains legacy/self-made Exact dashboard sections, but every active section and major drill-down is covered by the Stage 2 screenshot/interaction smoke. Studio Admin migration remains refactor work, not a Stage 2 release blocker. |
| Odoo addon test gate | Green via Ubuntu WSL | `wsl.exe -d Ubuntu -- bash -lc "cd '/mnt/c/Users/hassa/OneDrive/Desktop/Bayaan.ai/bayaan POS' && DROP_FAILED_DB=1 TEST_TIMEOUT_SECONDS=1800 bash scripts/odoo-addon-test.sh"` passed again on 2026-05-21 against disposable DB `bayaan_codex_20260521_032943`: 85 post-tests, 111 addon tests, 0 failed, 0 errors |
...
| Local pilot workflow gate | Green for local Stage 2 scope | Deterministic simulation/backend/frontend flows are passing, and legacy/self-made operational surfaces are explicitly covered by section-level screenshot/interaction smoke. Production deployment, hardware, accountant validation, and ops hardening remain outside this local gate. |
...
- Stage 3 refactor started with a behavior-preserving extraction of landed-cost orchestration from the large API controller into `backend/bayaan_odoo_addons/bayaan_fnb_kiosk/controllers/landed_cost_service.py`; the route still delegates through `/bayaan/api/landed_cost` and remains covered by the landed-cost and full addon gates.
- Stage 2 classic ERP gap closure is green locally: landed costs, offline opening/receipt semantics, source product images, peak-hour reporting with Baghdad-local day boundaries, analytic cost centers, pricelist/discount controls, Odoo void/invoice/period locks, and MRP/semi-finished goods.
