...
Purpose: preserve today's trace of the meeting requirements against the current Bayaan/Odoo codebase, then turn the gaps into a workflow-ready backlog. This document is not a release sign-off. It is a source audit; no frontend verify gate or Odoo addon test gate was run as part of this pass.
...
Bayaan is not yet fully covered for the classic ERP commitments from the meeting: manufacturing/semi-finished goods, landed costs, mandatory analytic cost centers, server-side pricelist enforcement, Odoo-level void/invoice/period locks, and live product-by-hour reporting still need explicit implementation or configuration proof. Evidence anchors for these gaps are listed below.
...
| Kiosk stock and branch identity | Covered for stock/POS, missing analytic | `bayaan.kiosk` links POS config and stock location: `backend/bayaan_odoo_addons/bayaan_fnb_kiosk/models/bayaan_kiosk.py:10`, validates POS source location: `backend/bayaan_odoo_addons/bayaan_fnb_kiosk/models/bayaan_kiosk.py:60`. | Add analytic account/cost center mapping to kiosk or confirmed Odoo configuration. |
...
| Analytic accounts / mandatory cost centers | Gap | Kiosk model fields cover POS config, stock location, users, and company but no analytic account field: `backend/bayaan_odoo_addons/bayaan_fnb_kiosk/models/bayaan_kiosk.py:10`. Current profit estimate is operational, based on sales less consumption/waste/variance, not Odoo analytic journal enforcement: `backend/bayaan_odoo_addons/bayaan_fnb_kiosk/controllers/api.py:4460`. | Add analytic account mapping, mandatory analytic validation for journal lines, and HQ analytic distribution configuration/tests. |
...
- Add/derive an analytic account for each `bayaan.kiosk`.
- Propagate kiosk analytic account to POS/accounting outputs where Odoo creates journal lines.
- Add HQ analytic account and analytic distribution templates for shared expenses.
- Add server-side validation or Odoo configuration that blocks journal lines without an analytic allocation except an explicit whitelist.

Acceptance criteria:
- Tests prove POS sale revenue/COGS is allocated to the kiosk analytic account.
- Tests prove manual journal entry without analytic account is blocked.
